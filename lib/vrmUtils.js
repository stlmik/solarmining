...
function getSunshineLevel(minutes) {
    var last15MinsEnd = getEnd15Minute(new Date(2021, 02, 26, 7));
    var last15MinsStart = last15MinsEnd - minutes*60 - 2;
    return new Promise(function(resolve, reject) {
        //TODO add timestamps for 15-mins
        var url = '/installations/' + VRM_INSTALLID + '/stats?start=' + last15MinsStart +'&end=' + last15MinsEnd + '&interval=15mins&type=solar_yield';
        //console.log(url);
        axios.get(url)
            .then(function (response) {
